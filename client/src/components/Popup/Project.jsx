...
const Project = ({ dataPopup }) => {
	...
	const createProject = () => {
		let data = {
			...
			projectManager: state.users.managers.map(manager => manager.email),
			projectMember: state.users.members.map(member => member.email)
		};
	...
	return (
		<div className='popup__project' ref={popup}>
			...
			<div className='popup__project__btn-box d-flex'>
				...
				<div className='project__btn' onClick={handleClosePopup}>
					Huỷ
				</div>
			</div>
		</div>
